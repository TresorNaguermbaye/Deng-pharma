...
import { Brain, Send, User, Bot, Sparkles, CloudRain, AlertTriangle, Package, TrendingUp } from "lucide-react";
...
export default function ChatPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Bonjour ! Je suis l'assistant IA de DENG PHARMA. Je peux vous aider sur :\n\n📊 **Prévisions de ventes**\n⚠️ **Détection de ruptures**\n📦 **Recommandations de commandes**\n🌧️ **Analyse saisonnière** (Tchad)\n💊 **Conseils sur les médicaments**\n\nQue voulez-vous savoir ?", timestamp: new Date() }
  ]);
  ...
  const suggestions = [
    { icon: CloudRain, text: "Analyse saisonnière actuelle" },
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const assistantMsg: Message = { role: "assistant", content: data.reply || "Désolé, je n'ai pas compris.", timestamp: new Date() };
      setMessages(prev => [...prev, assistantMsg]);
    } catch (err) {
      setMessages(prev => [...prev, { role: "assistant", content: "Désolé, le service IA est momentanément indisponible.", timestamp: new Date() }]);
    } finally {
      setLoading(false);
    }
  };
        ...
        <CardContent className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              {msg.role === "assistant" && (
                <div className="w-8 h-8 bg-gradient-to-br from-[#0ABAB5] to-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
        ...
        <div className="p-4 border-t">
          <div className="flex gap-3">
            <Input
              ...
              className="h-12"
            />
            <Button onClick={handleSend} disabled={loading || !input.trim()} className="h-12 px-6 bg-gradient-to-r from-[#0ABAB5] to-blue-600 text-white">
