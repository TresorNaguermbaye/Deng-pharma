...
import { Search, Plus, Minus, Trash2, ShoppingCart, Receipt, Download, FileText } from "lucide-react";
...
export default function SalesPage() {
  ...
  const downloadInvoice = async (saleId: number) => {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/sales/sales/${saleId}/invoice/`, {
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Gérez vos ventes et générez des factures</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <Card className="border-0 shadow-md">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><ShoppingCart className="w-5 h-5" /> Panier</CardTitle>
            </CardHeader>
            <CardContent>
              {cart.length === 0 ? (
                <p className="text-slate-400 text-center py-8">Panier vide. Recherchez un médicament.</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {cart.map(item => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          <Button variant="ghost" size="sm" className="text-red-500" onClick={() => removeFromCart(item.id)}><Trash2 className="w-4 h-4" /></Button>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
              ...
              {cart.length > 0 && (
                <div className="border-t pt-4 mt-4 space-y-2 text-right">
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <Label>Remise :</Label>
                    <Input type="number" value={discount} onChange={e => setDiscount(Number(e.target.value))} className="w-32" />
                    <span>FCFA</span>
                  </div>
          ...
          <Card className="border-0 shadow-md">
            ...
            <CardContent className="space-y-3">
              <div className="space-y-2">
                ...
                <Input value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="Client comptoir" />
              </div>
              <div className="space-y-2">
                  ...
                  {medicines.map(med => (
                    <div key={med.id} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg cursor-pointer hover:bg-slate-100" onClick={() => addToCart(med)}>
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{formatFCFA(med.selling_price)}</p>
                      </div>
