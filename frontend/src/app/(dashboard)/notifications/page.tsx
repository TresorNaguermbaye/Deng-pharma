    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">{unread} non lue(s)</p>
        </div>
        {unread > 0 && (
      ...
      <Card className="border-0 shadow-md">
        <CardContent className="p-0">
          {notifs.length === 0 ? (
            ...
            notifs.map((n: any) => (
              <div key={n.id} className={`flex items-start gap-4 p-4 border-b last:border-0 ${n.is_read ? 'bg-white' : 'bg-blue-50'}`}>
                {typeIcon(n.type)}
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <Badge className="text-xs">{typeLabel(n.type)}</Badge>
