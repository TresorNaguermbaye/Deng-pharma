    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-[#0F1A2C] p-4">
      ...
      <Card className="w-full max-w-md border-0 shadow-2xl bg-white/95 backdrop-blur-sm relative z-10">
        <CardHeader className="text-center space-y-4">
          ...
          <CardTitle className="text-2xl font-bold text-slate-800">DENG PHARMA</CardTitle>
          <CardDescription className="text-slate-500">Connexion à la plateforme</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              <Label>Nom d'utilisateur</Label>
              <Input value={username} onChange={(e) => setUsername(e.target.value)} required className="h-12" />
            </div>
            <div className="space-y-2">
            ...
            <Button type="submit" disabled={loading} className="w-full h-12 bg-gradient-to-r from-[#0ABAB5] to-blue-600 text-white font-semibold">
              {loading ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Connexion...</> : "Se connecter"}
            </Button>
          </form>
        </CardContent>
      </Card>
